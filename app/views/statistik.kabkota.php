...
(isset($_GET['id'])) ? $where = "id_provinsi = ".$_GET['id'] : $where = "";
...
<table id="statistik" class="table-statistik table-responsive table-condensed table-striped">
    ...
    <tbody>
        ...
        $i=1;
        $field = ['*'];
        $from = "statistik_kabupaten_kota";
        $join = "";
        $order = "nama_kabupaten_kota";
        $query = $crud->read($field, $from, $join, $where, $order);
        while($row = mysqli_fetch_assoc($query)) {
            ?>
            <tr>
                <td class="text-right"><?=$i++?></td>
                <td data-id="<?=$row['id_kabupaten_kota']?>" class="namaAct"><?=$row['nama_kabupaten_kota']?></td>
